...
function NavLink({ href, icon: Icon, label, active, aberta, tooltip, onNavigate }: NavLinkProps) {
  return (
    <Link
      href={href}
      data-active={active}
      ...
      className={cn(
        ...
        aberta
          ? "gap-2.5 pl-3 pr-2 min-h-10 md:min-h-8 rounded-[var(--tf-radius-sm)] text-[0.875rem] md:text-[0.8125rem]"
  ...
  useEffect(() => {
    if (pathname.startsWith("/quadro/")) {
      ...
      if (qId) {
        ...
        if (
          boardMatch &&
          boardMatch.workspace_id &&
          boardMatch.workspace_id !== activeWorkspaceId
        ) {
    ...
    <div className={cn("py-2.5 flex-1 flex flex-col sidebar-item", !aberta && "items-center")}>
      {showHeader && (
        ...
        >
          ...
          {onToggle && (
            <button
              onClick={onToggle}
              className="w-9 h-9 md:w-7 md:h-7 flex items-center justify-center rounded-[var(--tf-radius-xs)] sidebar-item hover:bg-[var(--tf-surface-hover)] transition-colors"
              style={{ color: "var(--tf-text-tertiary)" }}
      ...
      {!activeWorkspaceId && aberta && (
        <div className="flex-1 px-4 flex flex-col items-center justify-center text-center pb-10">
          ...
          <p
            className="text-[0.75rem] mt-1 mb-2"
...
export function Sidebar({ quadros, onNovoQuadro, aberta, onToggle }: SidebarProps) {
  ...
  // Mobile/tablet: usa Drawer à esquerda
  if (isTabletOrBelow) {
    return (
      <Drawer
        aberto={aberta}
        onFechar={onToggle}
        lado="left"
        larguraMax="min(86vw, 288px)"
      >
        <SidebarBody
          quadros={quadros}
          onNovoQuadro={onNovoQuadro}
          aberta={true}
          onNavigate={onToggle}
          showHeader={true}
        />
  ...
  // Desktop: aside fixo na grid
  return (
    <aside
      className={cn(
        "sidebar-ease flex flex-col shrink-0 overflow-y-auto overflow-x-hidden mt-3.5 mb-3 ml-3 rounded-[var(--tf-radius-xl)] relative",
        aberta ? "w-[232px]" : "w-[52px]"
      )}
      style={{
