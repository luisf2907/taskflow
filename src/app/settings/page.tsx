...
import { Bell, Copy, Plus, Terminal } from "lucide-react";
      ...
      <div className="flex-1 flex flex-col overflow-hidden px-2 lg:px-4">
        ...
        >
          <div className="max-w-xl mx-auto px-6 py-10 space-y-8">
            ...
            {/* ── Profile ── */}
            <ProfileSection perfil={perfil} userEmail={user?.email} onUpdate={refresh} />
            ...
            {/* ── Notificações ── */}
            <NotifSection userId={user?.id} perfilPrefs={perfil?.notif_preferences} />
...
const NOTIF_OPTIONS: { key: keyof NotifPreferences; label: string; desc: string }[] = [
  { key: "email_convite", label: "Convite de workspace", desc: "Receber email ao ser convidado para um workspace" },
  { key: "email_card_atribuido", label: "Card atribuido", desc: "Receber email quando um card for atribuido a voce" },
  { key: "email_digest_semanal", label: "Resumo semanal", desc: "Receber um resumo por email toda semana" },
  { key: "inapp_todas", label: "Notificacoes in-app", desc: "Mostrar notificacoes dentro da plataforma" },
];

function NotifSection({ userId, perfilPrefs }: { userId?: string; perfilPrefs?: NotifPreferences | null }) {
  const [prefs, setPrefs] = useState<NotifPreferences>(perfilPrefs || DEFAULT_PREFS);
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="rounded-[20px] p-6 space-y-1" style={{ background: "var(--tf-bg-secondary)" }}>
        {NOTIF_OPTIONS.map(({ key, label, desc }) => (
          ...
          >
            <div>
              <p className="text-[13px] font-semibold" style={{ color: "var(--tf-text)" }}>{label}</p>
              <p className="text-[12px] mt-0.5" style={{ color: "var(--tf-text-tertiary)" }}>{desc}</p>
            </div>
            <button
              onClick={() => togglePref(key)}
              className="relative w-[40px] h-[24px] rounded-full transition-colors duration-200 shrink-0"
              style={{ background: prefs[key] ? "var(--tf-accent)" : "var(--tf-border)" }}
...
function ProfileSection({ perfil, userEmail, onUpdate }: { perfil: import("@/types").Perfil | null; userEmail?: string; onUpdate: () => void }) {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="rounded-[20px] p-6" style={{ background: "var(--tf-bg-secondary)" }}>
        <div className="flex items-start gap-5">
          {perfil?.avatar_url ? (
            <img src={perfil.avatar_url} alt="" className="w-16 h-16 rounded-full shrink-0" />
          ) : (
            <div
          ...
          <div className="flex-1 min-w-0 space-y-3">
            {editando ? (
              <>
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wide mb-1 block" style={{ color: "var(--tf-text-tertiary)" }}>Nome</label>
                  <input
                    value={nome}
                ...
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wide mb-1 block" style={{ color: "var(--tf-text-tertiary)" }}>Email</label>
                  <p className="text-[13px] px-3 py-2 rounded-[10px]" style={{ background: "var(--tf-surface)", color: "var(--tf-text-tertiary)" }}>
                    {userEmail} <span className="text-[10px]">(nao editavel)</span>
                  </p>
                </div>
                {perfil?.github_username && (
                  <p className="text-[12px]" style={{ color: "var(--tf-text-tertiary)" }}>
